...
const pushTopOffset = (stackSrc, stackDst, value) => {
    while (stackSrc.top.value != value) {
        if (
            stackSrc.top.prev &&
            value == stackSrc.top?.prev?.value &&
            stackSrc.label == "b"
        )
            ops.swap(stackSrc);
        else ops.rotate(stackSrc);
    }
...
const pushChunk = (stackA, stackB, chunk) => {
    ...
    while (length--) {
        ...
        if (
            ops.topOffset(stackA.length, holdTop[0]) <=
            ops.bottomOffset(holdBottom[0])
        ) {
...
/*TESTS*/
const stackA = ops.newStack("a", input);
